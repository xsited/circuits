...
def checksum(source_string):
    ...
    print("source_string %s" % source_string)
    print("countTo %d" % countTo)
    # Handle bytes in pairs (decoding as short ints)
    loByte = 0
    hiByte = 0
    while count < countTo:
        if (sys.byteorder == "little"):
            loByte = int(source_string[count])
            hiByte = int(source_string[count + 1])
        else:
            loByte = int(source_string[count + 1])
            hiByte = int(source_string[count])
        print("loByte %d" % loByte)
	print("hiByte %d" % hiByte)
        sum = sum + (int(hiByte) * 256 + int(loByte))
        count += 2
