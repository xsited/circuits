...
def default_echo_handler(message, ovsconn):
    ...
    ovsconn.send({
		  "result": json.dumps(message.get("params", None),  indent=4, sort_keys=True),
...
class OVSDBConnection(threading.Thread):
    ...
    def send(self, message, callback=None):
        ...
        self.socket.send(json.dumps(message,  indent=4, sort_keys=True))
...
if __name__ == '__main__':
    ...
    def res(message, ovsconn):
        print "list_dbs_query response ", json.dumps(message, indent=4, sort_keys=True)
    ...
    def monitor_res(message, ovsconn):
        print "monitor response", json.dumps(message,  indent=4, sort_keys=True)
